...
const GLfloat Panel::panel_vertices[] = {
...
Panel::Panel()
{
	GLfloat *Panel_triangle = new GLfloat[84]();

	for (int i = 0; i < 2; i++)
	{
		TBNTriangle(&panel_vertices[24 * i], &Panel_triangle[42 * i]);
	}
	...
	// 向缓冲区中写入数据
	glBufferData(GL_ARRAY_BUFFER, 84 * sizeof(GLfloat), Panel_triangle, GL_STATIC_DRAW);
...
Panel::Panel(glm::vec3 position, glm::vec3 rotation, glm::vec3 scale)
{
	GLfloat *Panel_triangle = new GLfloat[84]();
...
void Panel::setRotation(glm::vec3 rotation)
{
	this->rotation = rotation;
}

void Panel::setScale(glm::vec3 scale)
{
	this->scale = scale;
}
...
void Panel::renderPanel(Shader* shader)
{
...
void Panel::TBNTriangle(const GLfloat * vertex_input, GLfloat * vertex_output)
{
	glm::vec3 pos[3] = {
